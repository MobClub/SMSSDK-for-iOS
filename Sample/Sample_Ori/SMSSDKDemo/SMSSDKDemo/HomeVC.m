...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (curStep==0) {
        curStep++;
        NSBundle *quartzCoreSupportBundle = [NSBundle bundleWithPath:@"/System/Library/Frameworks/AppTrackingTransparency.framework"];
        if (quartzCoreSupportBundle != nil) {
            ...
            [NSClassFromString(@"ATTrackingManager") performSelector:@selector(requestTrackingAuthorizationWithCompletionHandler:) withObject:block];
        } else {
...
{
    SMSDemoCommitCodeVC *vc = [[SMSDemoCommitCodeVC alloc] initWithMethod:SMSGetCodeMethodSMS
                                                                 template:nil
                                                                   result:^(NSDictionary * _Nonnull dict, NSError * _Nonnull error) {
