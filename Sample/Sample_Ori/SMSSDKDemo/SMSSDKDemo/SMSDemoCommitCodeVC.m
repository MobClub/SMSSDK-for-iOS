...
typedef NS_ENUM(NSUInteger, MobileVerifyFlow) {
    AutoMobileVerify,
    SMSMobileVerify,
    VoiceMobileVerify,
    Unknown
...
// 当前所在状态
@property (nonatomic, assign) MobileVerifyFlow currentState;
...
//verifyBtn Top
@property (nonatomic, strong) NSLayoutConstraint *verifyBtnTopConstraint;
...
- (void)viewDidLoad {
    ...
    //默认为手机认证状态
    self.currentState = AutoMobileVerify;
...
//           NSLayoutConstraint *topConstraint = [NSLayoutConstraint constraintWithItem:self.codeTitleLbl attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.codeEditView attribute:NSLayoutAttributeTop multiplier:1.0 constant:15];
    ...
    {
        NSLayoutConstraint *topConstraint = [NSLayoutConstraint constraintWithItem:self.verifyBtn attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.codeEditView attribute:NSLayoutAttributeTop multiplier:1.0 constant:30];
        ...
        _verifyBtnTopConstraint = topConstraint;
    }
...
//    self.bgView.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height);
    ...
    _phone = phoneNum;
    if (_currentState == AutoMobileVerify) {
        ...
        [SMSDemoProcessHUD showProcessHUDWithInfo:SMSDemoLocalized(@"commitingCode")];
        [SMSSDK commitVerificationCode:code phoneNumber:_phone zone:_zone result:^(NSError *error) {
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if(textField == self.phoneTf)
    {
        ...
        if(lastStr.length > 0 && _codeTf.text.length >= 4 && _currentState != AutoMobileVerify)
        {
            _verifyBtn.enabled= YES;
        }
        else if ([lastStr length] > 0 && _currentState == AutoMobileVerify)
        {
            _verifyBtn.enabled= YES;
        } else {
            _verifyBtn.enabled = NO;
...
#pragma mark - Update Subviews Constraints
- (void)updateSomeSubviewsConstraints {
    __weak typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_main_queue(), ^{
        if (weakSelf.currentState == AutoMobileVerify) {
            return;
        }
        
        weakSelf.verifyBtn.enabled = NO;
        weakSelf.codeEditView.userInteractionEnabled = YES;
        [UIView animateWithDuration:0.4 animations:^{
            weakSelf.codeEditView.hidden = NO;
            weakSelf.verifyBtnTopConstraint.constant = 112;
        } completion:^(BOOL finished) {
            [weakSelf.view layoutIfNeeded];
            if (weakSelf.methodType == SMSGetCodeMethodSMS) {
                [weakSelf sendTextCode];
            } else {
